...
private:
	...
	void addCompulsiveForce(const std::vector<point_t> &points, index_t p1, index_t p2, length_t length, std::vector<point_t> &forces)
	{
		...
		real_t fact = (real_t)std::sqrt(sqLen) * edgeCompulsion / (real_t)(length);
		dx *= fact;
		dy *= fact;
		forces[p1].x() += dx;
		forces[p1].y() += dy;
...
public:
	SerialSimulator(index_t pointCount, const std::vector<edge_t> &edges, const std::vector<length_t> &lengths)
		: mEdges(edges), mLengths(lengths)
	{
	...
	void computeForces(std::vector<point_t> &points, std::vector<point_t> &forces)
	{
		...
		for (std::size_t i = 0; i < mEdges.size(); ++i)
			addCompulsiveForce(points, mEdges[i].x(), mEdges[i].y(), mLengths[i], forces);
	}
