...
void DrawableObject::fill_texture(State* p_state)
{
    if(_changed) {
        ...
        //std::vector<unsigned char> pixels(BYTES_PER_PIXEL * PLOT_WIDTH * PLOT_HEIGHT, DEFAULT_WHITE);
        std::fill(p_state->pixels.begin(), p_state->pixels.end(), DEFAULT_WHITE);
    ...
    // pixels = std::vector<unsigned char>(BYTES_PER_PIXEL * PLOT_WIDTH * PLOT_HEIGHT, 63);
        ...
        ImageView2D image{ GL::PixelFormat::RGBA, GL::PixelType::UnsignedByte, { PLOT_WIDTH, PLOT_HEIGHT },
            { p_state->pixels.data(), std::size_t(BYTES_PER_PIXEL * PLOT_WIDTH * PLOT_HEIGHT) } };
...
Canvas::Canvas(Object3D& parent, SceneGraph::DrawableGroup3D& drawables)
{
    ...
    _canvas = new DrawableObject{0, _textured_shader, _canvas_mesh, parent, drawables};
    (*_canvas)//.translate({0.0f, 0.0f, -2000.0f})
            .scale(Vector3{PLOT_WIDTH / 2.0f, PLOT_HEIGHT / 2.0f, 0.0f});//.rotateX(-90.0_degf)        
