...
struct Point
{
    ...
    int get_max(int value) { return std::max(value, std::max(x, y)); }
    int get_min(int value) { return std::min(value, std::min(x, y)); }
};

// Normal distribution
std::pair<int, int>
fill_vector(std::vector<Point> &result, int cell_cnt, int mean, int std_dev)
{
...
void
fill_pixels(std::vector<unsigned char> &pixels,
            int cell_cnt,
            int mean,
            int std_dev)
{
    ...
    for (auto &&point : points) {
        ...
        if (point.y < min)
            point.y = min + 1;
        std::size_t x_plot =
          (std::size_t)shift_interval(point.x, min, max, 0, PLOT_WIDTH - 1);
        std::size_t y_plot =
          (std::size_t)shift_interval(point.y, min, max, 0, PLOT_HEIGHT - 1);
